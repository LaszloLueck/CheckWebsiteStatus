    ...
    public class SchedulerJob : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            await Task.Run(async () =>
            {
                ...
                var t = Task.WhenAll(ReadAndProcessSubElements(cssNodes.Concat(srcNodes)))
                     .ContinueWith(_ => Logger.Log("Finished Task!"));

                await t;
            });
        }

        private async Task<IEnumerable<HtmlNode>> GetResultFromUrl(string url)
        {
            ...
            var tl = nodes.Select(htmlNode =>
            {
                return htmlNode.Name.ToLower() switch
                {
                    ...
                    "link" => GetResponseForSubElement(htmlNode.Attributes["href"].Value, headerAcceptCss,
                        ElementType.Text),
                    "script" => GetResponseForSubElement(htmlNode.Attributes["scr"].Value, headerAcceptJs,
                        ElementType.Text),
                    _ => Task.Run(() => { })
                };
            });
        ...
        private async Task GetResponseForSubElement(string url, string acceptHeader, ElementType elementType)
        {
